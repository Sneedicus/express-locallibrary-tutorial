...
exports.bookinstance_delete_get = function(req, res, next) {
    BookInstance.findById(req.params.id, function(err, book_instance) {
            if(err) { return next(err); }
...
exports.bookinstance_update_get = function(req, res, next) {
    async.parallel(
        {
            book_instance: function(callback) { BookInstance.findById(req.params.id).populate('book').exec(callback) },
            book_list: function(callback) { Book.find(callback) }
        },
        function(err, results)
        {
            if(err) { return next(err); }
            res.render('bookinstance_form', { title: 'Update BookInstance', book_instance: results.book_instance, selected_book: results.book_instance.book._id, book_list: results.book_list });
        }
    );
};
    ...
    (req, res, next) => {
        ...
        if(!errors.isEmpty()) {
            ...
            async.parallel(
                ...
                {
                    if(err) { return next(err); }
                    res.render('bookinstance_form', { title: 'Update BookInstance', book_instance: book_instance, book_list: results.book_list });
                }
            );
            return;
        }
