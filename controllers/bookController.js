    ...
    async.parallel(
        {
            ...
            books_instances: function(callback) { BookInstance.find({ 'book': req.params.id }).exec(callback); }
        },
        function(err, results)
        {
            if(err) { return next(err); }
            if(results.book == null) { 
                res.redirect('/catalog/books'); // No results
            }
            // Successful, so render.
...
exports.book_delete_post = function(req, res, next) {
    async.parallel(
        {
            book: function(callback) { Book.findById(req.body.bookid).exec(callback); },
            books_instances: function(callback) { BookInstance.find({ 'book': req.body.bookid }).exec(callback); }
        },
        function(err, results)
        {
            if(err) { return next(err); }
            if(results.books_instances.length > 0) {
                // There are Book Instances that need deleting
                res.render('book_delete', { title: 'Delete Book', book: results.book, book_instances: results.books_instances });
                return;
            }
...
exports.book_update_post = [
    ...
    (req, res, next) => {
        ...
        // Create a Book object with escapes/trimmed data and old id.
        var book = new Book(
            {
                title: req.body.title,
                author: req.body.author,
